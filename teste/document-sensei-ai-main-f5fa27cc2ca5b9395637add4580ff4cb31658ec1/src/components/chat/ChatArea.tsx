...
const ChatArea = ({
  ...
  const [tipoSelecionado, setTipoSelecionado] = useState(tipos[0]?.id || 1);
  const promptSelecionado = tipos.find(t => t.id === tipoSelecionado)?.prompt || tipos[0]?.prompt || "";
        ...
        {!currentConversation ? (
          ...
          <div className="max-w-4xl mx-auto w-full space-y-6 p-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-4 mb-2">
              <label htmlFor="tipo-documento" className="font-medium text-[#0A2540]">Tipo de Documento:</label>
              <select
                id="tipo-documento"
                className="border rounded-md px-3 py-2 text-base focus:outline-none focus:ring-2 focus:ring-[#D2B07F]"
              ...
              <Button
                variant="outline"
                className="flex-shrink-0 px-4 py-2 bg-white border border-[#D2B07F] text-[#0A2540] rounded-xl hover:bg-[#FAF8F5] font-semibold"
                onClick={() => onSendMessage(promptSelecionado, true)}
                disabled={isLoading}
              >
                Extrair informações
              </Button>
            </div>
            {/* Botão de extração dinâmico */}
            {/* <ExtractFichaButton onExtract={(prompt) => onSendMessage(prompt, true)} prompt={promptSelecionado} /> */}
            <MessageList 
              messages={messages} 
              isLoading={isLoading}
